// app/api/v1/properties/lifestyle/route.ts - COMPLETE FIXED VERSION

import { NextRequest, NextResponse } from 'next/server';
import {
  getLifestyleProperties,
  getLifestylePropertyBySlug,
  getLifestylePropertyById,
  getFeaturedLifestyle,
  getRecentLifestyle,
  getLifestyleByBedroom,
  getLifestyleByPriceRange,
  type LifestyleFilters,
} from '@/lib/models/lifestyle-properties';
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (action === 'featured') {
      ...
      const result = await getFeaturedLifestyle(featuredLimit);
    ...
    if (action === 'recent') {
      ...
      const result = await getRecentLifestyle(recentLimit);
    ...
    if (id) {
      ...
      const property = await getLifestylePropertyById(propertyId);
      if (!property) {
        return NextResponse.json(
          { success: false, error: 'Lifestyle property not found' },
          { status: 404 }
    ...
    if (validMin > 0 || validMax > 0) {
      result = await getLifestyleByPriceRange(validMin, validMax, validLimit);
    } else if (bedroom) {
      result = await getLifestyleByBedroom(bedroom, validLimit);
    } else {
      result = await getLifestyleProperties(filters);
    }
    ...
    return NextResponse.json(
      {
        success: false,
        message: 'Failed to fetch lifestyle properties',
        error: error.message,
      },
      { status: 500 }
    );
