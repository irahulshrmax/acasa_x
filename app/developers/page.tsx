...
function DeveloperCard({ developer, index = 0 }: { developer: Developer; index?: number }) {
  const [imageError, setImageError] = useState(false);
  const imageSrc = useMemo(() => {
    if (imageError) return "https://images.unsplash.com/photo-1560518883-ce09059eeffa?w=400&h=300&fit=crop";
    return getDeveloperImage(developer);
  }, [developer, imageError]);

  const country = developer.country || "International";
  ...
  return (
    ...
    >
      <Link href={`/developers/${developer.seo_slug || developer.id}`} className="block">
        <div className="relative aspect-[4/3] overflow-hidden bg-gray-100">
          ...
          <div className="absolute top-3 left-3 flex flex-col gap-1.5">
            {totalProjects > 0 && (
              <span className="rounded-[3px] bg-[#0F1C2E]/80 px-2.5 py-1 text-[9px] font-medium uppercase tracking-[0.12em] text-white backdrop-blur-sm">
                <Building className="inline h-3 w-3 mr-1" />
                {totalProjects} Projects
              </span>
            )}
            {totalProperties > 0 && (
              <span className="rounded-[3px] bg-emerald-500/80 px-2.5 py-1 text-[9px] font-medium uppercase tracking-[0.12em] text-white backdrop-blur-sm">
          ...
          <div className="absolute right-3 top-3 rounded-[3px] bg-black/60 px-2 py-1 text-[9px] text-white backdrop-blur-sm">
        ...
        <div className="p-4">
          ...
          {developer.informations && (
            <p className="mt-2 line-clamp-2 text-[11px] leading-relaxed text-[#6B7A8D]">
              {developer.informations.replace(/<[^>]*>/g, '').slice(0, 120)}
              {developer.informations.replace(/<[^>]*>/g, '').length > 120 && "..."}
            </p>
          )}

          <div className="mt-3 flex flex-wrap items-center justify-between gap-2 border-t pt-3" style={{ borderColor: THEME.border }}>
            ...
            <span className="text-[9px] font-medium uppercase tracking-[0.12em] text-[#C9A96E]">
              View Details →
            </span>
          </div>
        </div>
...
export default function DevelopersPage() {
  ...
  const hasActiveFilters = filters.keyword !== "" || searchTerm !== "";
  ...
  const fetchDevelopers = useCallback(
    async (page: number, isReset: boolean) => {
      ...
      try {
        ...
        if (!data.success) throw new Error(data.error || "Failed to fetch developers");

        let newDevelopers: Developer[] = data.data || [];

        const seen = new Set<number>();
        newDevelopers = newDevelopers.filter((d) => {
        ...
        if (isReset) {
          ...
          setDevelopers((prev) => {
            ...
            return [...prev, ...newDevelopers.filter((d) => !existingIds.has(d.id))];
          });
        }
  ...
  useEffect(() => {
    fetchDevelopers(filters.page, filters.page === 1);
  }, [fetchDevelopers, filters.page]);
        ...
        <div className="mb-6">
          ...
          >
            {loading ? (
              ...
              <>
                <span>{pagination.total.toLocaleString()} developers</span>
                {isCached && (
          ...
          >
            <div className="p-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <button
                  onClick={handleClearFilters}
                  className="h-9 border border-gray-300 text-[10px] uppercase tracking-[0.12em] text-gray-700"
                >
                  Clear All
                </button>
              </div>
      ...
      <div className="mx-auto max-w-[1200px] px-4 pb-16 pt-6 md:px-6">
        {error && !loading && (
          <div className="mb-6 flex items-center gap-3 border border-red-200 bg-red-50 p-4 text-[13px] text-red-600">
            ...
            <span>{error}</span>
            <button
              onClick={() => fetchDevelopers(filters.page, filters.page === 1)}
        ...
        <div className="grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-3">
          {loading && developers.length === 0
            ...
            : developers.map((developer, index) => (
                <DeveloperCard key={`${developer.id}-${developer.seo_slug}`} developer={developer} index={index} />
              ))}
        </div>
