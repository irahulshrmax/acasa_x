...
function SimilarCard({ property, index }: { property: any; index: number }) {
  ...
  return (
    <motion.div
      ...
      onClick={() => router.push(`/archive-properties/${property.slug}`)}
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!res.ok || !data.success) throw new Error(data.message || "Archive property not found");
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-white">
        <div className="text-center px-4">
          ...
          <p className="text-sm text-red-500 mb-6">{error || "Archive property not found"}</p>
          <Link
        ...
        <div className="mx-auto max-w-[1180px] px-4 py-3 md:px-6">
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-1.5 text-[10px] uppercase tracking-[0.1em]" style={{ color: THEME.muted }}>
              ...
              <Link href="/archive-properties" className="hover:text-gray-600 transition-colors">
                Archive Properties
              </Link>
              <ChevronLeft className="h-3 w-3 rotate-180" />
        ...
        <div className="flex flex-wrap items-start justify-between gap-6">
          <div className="flex-1 min-w-0">
            <div className="flex flex-wrap gap-2 mb-3">
              ...
              <span
                className={`px-2.5 py-1 text-[8px] font-medium uppercase tracking-[0.15em] text-white ${statusColor}`}
              >
                {statusLabel}
              </span>
            ...
            <div className="mt-2 flex flex-wrap items-center gap-3 text-[12px]" style={{ color: THEME.muted }}>
              ...
              {property.created_at && (
                <div className="flex items-center gap-1.5">
                  ...
                  <span>Archived {new Date(property.created_at).toLocaleDateString()}</span>
                </div>
              )}
            </div>
                ...
                >
                  ...
                  {property.description ? (
                    ...
                    <p className="text-[13px] leading-relaxed" style={{ color: "#4A5462" }}>
                      Welcome to {property.property_name}, a property in {location}, Dubai.
                    </p>
                  )}
          ...
          <aside id="enquiry">
            <div className="sticky top-6">
              ...
              {property.can_restore && (
                <button
                  onClick={async () => {
                    if (confirm("Restore this property?")) {
                      try {
                        ...
                        if (res.ok) {
                          router.push("/archive-properties");
                        }
                      } catch {}
                    }
                  }}
              ...
              {property.can_permanently_delete && (
                <button
                  onClick={async () => {
                    if (confirm("⚠️ Permanently delete this property? This cannot be undone!")) {
                      try {
                        const res = await fetch(`${API_URL}/${property.id}?confirm=true`, {
                          method: "DELETE",
                        });
                        if (res.ok) {
                          router.push("/archive-properties");
