...
function JobCard({ job, index = 0 }: { job: Job; index?: number }) {
        ...
        <div className="p-6">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div className="flex-1 min-w-0">
              <h3
                className="text-[16px] font-normal uppercase leading-snug tracking-[0.06em] transition-opacity group-hover:opacity-70"
            ...
            <span
              className={`shrink-0 rounded-[3px] px-2.5 py-1 text-[8px] font-medium uppercase tracking-[0.12em] border ${typeColor}`}
            >
              {job.type || "Full-time"}
            </span>
          </div>
          ...
          {description && (
            <p className="mt-3 text-[13px] leading-relaxed text-[#4A5462]">
          ...
          <div className="mt-4 flex flex-wrap items-center justify-between gap-3 border-t pt-4" style={{ borderColor: THEME.border }}>
...
export default function CareersPage() {
  ...
  const hasActiveFilters = filters.type !== "" || searchTerm !== "";
        ...
        <div className="mb-6">
          ...
          >
            {loading ? (
              ...
              <>
                <span>{pagination.total.toLocaleString()} open positions</span>
                {isCached && (
      ...
      <div className="mx-auto max-w-[1200px] px-4 pb-16 pt-6 md:px-6">
        {error && !loading && (
          <div className="mb-6 flex items-center gap-3 border border-red-200 bg-red-50 p-4 text-[13px] text-red-600">
            ...
            <span>{error}</span>
            <button
              onClick={() => fetchJobs(filters.page, filters.page === 1)}
        ...
        <div className="grid grid-cols-1 gap-6">
          {loading && jobs.length === 0
            ...
            : jobs.map((job, index) => (
                <JobCard key={`${job.id}-${job.link}`} job={job} index={index} />
              ))}
        </div>
        ...
        {!loading && jobs.length === 0 && !error && (
          ...
          >
            ...
            <p className="mx-auto mt-2 max-w-md text-[13px] text-[#6B7A8D]">
              Check back later for new opportunities at ACASA.
            </p>
            <button
              onClick={handleClearFilters}
