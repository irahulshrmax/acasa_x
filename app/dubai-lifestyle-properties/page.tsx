  ...
  return (
    ...
    >
      <div className={`relative overflow-hidden bg-gray-100 ${isListMode ? "sm:w-[320px] sm:flex-shrink-0" : "aspect-[4/3]"}`}>
        <Link href={`/dubai-lifestyle-properties/${property.slug}`} className="block h-full w-full">
          {currentImage ? (
            <>
              <img
                src={currentImage}
        ...
        <div className="absolute left-3 top-3 flex flex-col gap-1.5">
          ...
          <span className="flex items-center gap-1 rounded-[3px] bg-amber-500 px-2.5 py-1 text-[9px] font-medium uppercase tracking-[0.12em] text-white">
      ...
      <div className={`flex flex-1 flex-col ${isListMode ? "py-2 pr-2" : "pt-4"}`}>
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0 flex-1">
            <Link href={`/dubai-lifestyle-properties/${property.slug}`}>
              <h3
...
export default function LifestylePropertiesPage() {
  ...
  const hasActiveFilters = filters.bedroom !== "" || filters.min_price !== "" || filters.max_price !== "" || filters.keyword !== "";
  ...
  const fetchProperties = useCallback(
    async (page: number, isReset: boolean) => {
      ...
      try {
        ...
        if (!data.success) throw new Error(data.error || "Failed to fetch lifestyle properties");
        ...
        setError(err.message || "Failed to load lifestyle properties");
        if (isReset) {
          setProperties([]);
          setIsInitialLoad(false);
        }
    ...
    <section className="min-h-screen bg-white" style={{ fontFamily: FONT_BODY }}>
        ...
        <div className="mb-6">
          ...
          >
            {loading ? (
              ...
              <>
                <span>{pagination.total.toLocaleString()} luxury lifestyle properties</span>
                {isCached && (
      ...
      <div className="mx-auto max-w-[1200px] px-4 pb-16 pt-6 md:px-6">
        ...
        {!loading && properties.length === 0 && !error && (
          ...
          >
            <Crown className="mx-auto h-14 w-14 text-gray-300" />
            <h3 className="mt-4 text-[22px] text-[#0F1C2E]" style={{ fontFamily: FONT_DISPLAY }}>
              No lifestyle properties found
            </h3>
