  ...
  return (
    ...
    >
      <div className={`relative overflow-hidden bg-gray-100 ${isListMode ? "sm:w-[320px] sm:flex-shrink-0" : "aspect-[4/3]"}`}>
        <Link href={`/off-plan-properties-in-dubai/${property.slug}`} className="block h-full w-full">
          {currentImage ? (
            <>
              <img
                src={currentImage}
      ...
      <div className={`flex flex-1 flex-col ${isListMode ? "py-2 pr-2" : "pt-4"}`}>
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0 flex-1">
            <Link href={`/off-plan-properties-in-dubai/${property.slug}`}>
              <h3
  ...
  const fetchProperties = useCallback(
    async (page: number, isReset: boolean) => {
      ...
      try {
        ...
        if (!data.success) throw new Error(data.error || "Failed to fetch offplan properties");
        ...
        setError(err.message || "Failed to load offplan properties");
        if (isReset) {
          setProperties([]);
          setIsInitialLoad(false);
        }
        ...
        <div className="mb-6">
          ...
          >
            {loading ? (
              ...
              <>
                <span>{pagination.total.toLocaleString()} off plan properties available</span>
                {isCached && (
