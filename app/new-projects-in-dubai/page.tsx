...
function getAllImages(project: Project): string[] {
  ...
  if (images.length === 0) {
    images.push("https://images.unsplash.com/photo-1560518883-ce09059eeffa?w=600&h=400&fit=crop");
  }
  return images;
}
...
function getBedroomLabel(bedroom: string | null): string {
...
function ProjectCard({ project, viewMode = "grid", index = 0 }: { project: Project; viewMode?: string; index?: number }) {
  ...
  const priceDisplay = project.price_display || formatPrice(project.price);
  const bedroomLabel = getBedroomLabel(project.bedroom);
  const daysAgo = getDaysAgo(project.created_at);
  const isOffPlan = project.listing_type === "Off plan";

  const allImages = useMemo(() => getAllImages(project), [project]);
  ...
  const specs = [
    bedroomLabel,
    project.area_display && project.area_display !== "Area on Request" ? project.area_display : null,
  ].filter(Boolean).join("  |  ");
  ...
  return (
    ...
    >
      <div className={`relative overflow-hidden bg-gray-100 ${isListMode ? "sm:w-[320px] sm:flex-shrink-0" : "aspect-[4/3]"}`}>
        <Link href={`/new-projects-in-dubai/${project.project_slug || project.id}`} className="block h-full w-full">
          {currentImage ? (
            <>
              <img
                src={currentImage}
        ...
        <div className="absolute left-3 top-3 flex flex-col gap-1.5">
          ...
          {project.listing_type && !isOffPlan && (
      ...
      <div className={`flex flex-1 flex-col ${isListMode ? "py-2 pr-2" : "pt-4"}`}>
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0 flex-1">
            <Link href={`/new-projects-in-dubai/${project.project_slug || project.id}`}>
              <h3
          ...
          <div className="shrink-0 text-right">
            <p className="text-[9px] font-medium uppercase tracking-[0.14em] text-[#6B7A8D]">Starting Price</p>
        ...
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-2">
            {project.dld_permit && (
              <span className="flex items-center gap-1 rounded-full bg-orange-50 px-2 py-0.5 text-[9px] font-medium text-orange-700">
        ...
        {isListMode && project.description && (
          <p className="mt-2 line-clamp-2 text-[11px] text-[#6B7A8D]">
            {stripHtml(project.description).slice(0, 120)}
            {stripHtml(project.description).length > 120 && "..."}
          </p>
        )}
      </div>
    </motion.div>
...
export default function ProjectsPage() {
  ...
  const hasActiveFilters = filters.bedroom !== "" || filters.min_price !== "" || filters.max_price !== "" || filters.listing_type !== "" || searchTerm !== "";
      ...
      try {
        ...
        if (filters.bedroom) {
          if (filters.bedroom.toLowerCase() === "studio") {
            params.append("bedroom", "Studio");
          } else {
            params.append("bedroom", filters.bedroom);
          }
        }
        ...
        let newProjects: Project[] = data.data || [];
  ...
  useEffect(() => {
    fetchProjects(filters.page, filters.page === 1);
        ...
        <div className="mb-6">
          ...
          >
            {loading ? (
              ...
              <>
                <span>{pagination.total.toLocaleString()} projects available</span>
                {isCached && (
      ...
      <div className="mx-auto max-w-[1200px] px-4 pb-16 pt-6 md:px-6">
        {error && !loading && (
          <div className="mb-6 flex items-center gap-3 border border-red-200 bg-red-50 p-4 text-[13px] text-red-600">
            ...
            <span>{error}</span>
            <button
              onClick={() => fetchProjects(filters.page, filters.page === 1)}
        ...
        >
          {loading && projects.length === 0
            ...
            : projects.map((project, index) => (
                <ProjectCard key={`${project.id}-${project.project_slug}`} project={project} viewMode={viewMode} index={index} />
              ))}
        </div>
