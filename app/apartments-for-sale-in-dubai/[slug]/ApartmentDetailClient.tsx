  ...
  return (
    ...
    >
      <div className="flex items-center justify-between px-6 py-4">
        ...
        <button onClick={onClose} className="flex h-10 w-10 items-center justify-center rounded-full bg-white/10 text-white transition-colors hover:bg-white/20">
          <X className="h-5 w-5" />
        </button>
      </div>
...
function SimilarCard({ property, index }: { property: any; index: number }) {
  ...
  const img = !imgErr ? (property.featured_image || property.image_url || property.gallery_urls?.[0] || null) : null;
  ...
  return (
    <motion.div
      ...
      onClick={() => router.push(`/apartments-for-sale-in-dubai/${property.property_slug || property.slug}`)}
    ...
    >
      <div className="relative aspect-[4/3] overflow-hidden bg-gray-100">
        {img ? (
          ...
          <div className="flex h-full w-full items-center justify-center bg-gray-50"><Building2 className="h-10 w-10 text-gray-300" /></div>
        )}
        <div className="absolute inset-0 bg-gradient-to-t from-black/50 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
      </div>
      <div className="mt-4 space-y-1">
        <h3 className="truncate text-[13px] uppercase tracking-wide transition-colors group-hover:text-[#C8AA78]" style={{ fontFamily: FONT_DISPLAY, color: THEME.primary }}>
          {property.property_name || property.name}
        </h3>
        <p className="flex items-center gap-1 text-[11px]" style={{ color: THEME.muted }}><MapPin className="h-3 w-3" /> {property.location?.city || 'Dubai'}</p>
        <p className="text-[13px] font-semibold" style={{ color: THEME.primary }}>{getDisplayPrice(property.price)}</p>
      </div>
    </motion.div>
  );
}
...
export default function ApartmentDetailClient() {
  ...
  useEffect(() => {
    ...
    const fetchSimilar = async () => {
      ...
      try {
        ...
        const data = await res.json();
        if (data.success && data.data) {
          setSimilarProperties(data.data.filter((p: any) => p.id !== property.id).slice(0, 3));
        }
      } catch (err) {
  ...
  const galleryImages = useMemo(() => {
    return getAllGalleryImages(property);
  }, [property]);
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-white">
        <div className="text-center px-4 max-w-md">
          ...
          <h1 className="text-2xl font-light text-[#192334]" style={{ fontFamily: FONT_DISPLAY }}>Not Found</h1>
          <p className="text-sm text-red-500 my-4">{error || 'Property not found'}</p>
          <div className="mt-4 p-4 bg-gray-50 rounded text-xs text-left font-mono overflow-auto max-h-32 border border-gray-200">
            <p><strong>Slug:</strong> {slug}</p>
            <p><strong>API:</strong> /api/v1/properties/{slug}</p>
          </div>
          <div className="mt-6 flex flex-wrap justify-center gap-3">
            <button
              onClick={() => { fetchedRef.current = false; setLoading(true); setError(null); window.location.reload(); }}
              className="bg-[#192334] px-6 py-2.5 text-[11px] tracking-widest text-white hover:bg-[#2a3a4a]"
            >
              <ArrowLeft className="inline h-4 w-4 mr-2" /> RETRY
            </button>
            <Link href="/apartments-for-sale-in-dubai" className="bg-gray-200 px-6 py-2.5 text-[11px] tracking-widest text-gray-700 hover:bg-gray-300">
              All Apartments
            </Link>
          </div>
        </div>
    ...
    <div className="min-h-screen bg-white" style={{ fontFamily: FONT_BODY }}>
        ...
        <div className="mx-auto max-w-[1180px] px-4 py-3 md:px-6">
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-1.5 text-[10px] uppercase tracking-[0.1em]" style={{ color: THEME.muted }}>
              ...
              <Link href="/apartments-for-sale-in-dubai" className="hover:text-gray-600 transition-colors">Apartments</Link>
              <ChevronLeft className="h-3 w-3 rotate-180" />
              <span className="truncate max-w-[120px] sm:max-w-none text-gray-500">{displayName}</span>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={handleShare} className="flex h-9 w-9 items-center justify-center border border-gray-200 bg-white text-gray-500 transition-all hover:border-gray-300 hover:text-gray-700">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex flex-wrap gap-2 mb-3">
              ...
              {property.listing_type && (
                <span className="px-2.5 py-1 text-[8px] font-medium uppercase tracking-[0.15em] border" style={{ borderColor: THEME.border, color: THEME.muted }}>{property.listing_type}</span>
              )}
              ...
              {property.occupancy && (
                <span className="px-2.5 py-1 text-[8px] font-medium uppercase tracking-[0.15em] bg-blue-50 text-blue-700">{occupancyLabel}</span>
              )}
            </div>
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          ...
          {property.completion_date && (
            <div className="flex items-center gap-2 rounded-[3px] border px-3.5 py-2" style={{ borderColor: THEME.border }}>
              ...
              <span className="text-[11px] font-medium" style={{ color: THEME.primary }}>{completionStatus}</span>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="mx-auto max-w-[1180px] px-4 md:px-6">
        <div className="relative overflow-hidden bg-gray-100 aspect-[16/9] group cursor-pointer" onClick={() => setShowModal(true)}>
          {mainImage ? (
            <img src={mainImage} alt={displayName} className="h-full w-full object-cover transition-transform duration-700 group-hover:scale-[1.02]" />
          ) : (
          ...
          <div className="absolute bottom-4 left-4 flex gap-2">
            {hasGallery && (
              <button onClick={e => { e.stopPropagation(); setShowModal(true); }} className="flex items-center gap-1.5 bg-white/95 px-4 py-2 text-[10px] font-medium uppercase tracking-[0.15em] text-gray-800 backdrop-blur-sm transition-colors hover:bg-white">
                <Grid3x3 className="h-3.5 w-3.5" /> All Photos ({galleryImages.length})
              </button>
            )}
            {property.video_url && (
              <a href={property.video_url} target="_blank" rel="noreferrer" onClick={e => e.stopPropagation()} className="flex items-center gap-1.5 bg-black/60 px-4 py-2 text-[10px] font-medium uppercase tracking-[0.15em] text-white backdrop-blur-sm transition-colors hover:bg-black/80">
          ...
          <div className="mt-2">
            <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
              ...
              {!showAllPhotos && galleryImages.length > 10 && (
                ...
                >
                  <Plus className="h-3.5 w-3.5 mr-1" /> {galleryImages.length - 10}
                </button>
              )}
            </div>
          </div>
              ...
              {activeTab === 'overview' && (
                ...
                >
                  <h2 className="text-[20px] sm:text-[24px] leading-snug mb-4" style={{ fontFamily: FONT_DISPLAY, color: THEME.primary }}>
                    {property.display_title || displayName}
                    {property.developer?.name && (
                      <span className="block text-[16px] mt-1" style={{ color: THEME.muted }}>by {property.developer.name}</span>
                    )}
                  </h2>
                  ...
                  {property.description ? (
                    ...
                    <p className="text-[13px] leading-relaxed" style={{ color: '#4A5462' }}>
                      Welcome to {displayName}, a distinguished residential development in {location}, Dubai.
                    </p>
                  )}
                  ...
                  {property.developer?.name && (
                    <div className="mt-8 border-t pt-8" style={{ borderColor: THEME.border }}>
                      <p className="text-[9px] uppercase tracking-[0.2em] mb-4" style={{ color: THEME.muted }}>Developer</p>
                      <div className="flex items-center gap-4">
                        {property.developer.logo && (
                          <div className="flex h-14 w-20 items-center justify-center border rounded-[3px] p-2" style={{ borderColor: THEME.border }}>
                            <img src={fixImageUrl(property.developer.logo)} alt={property.developer.name} className="max-h-full max-w-full object-contain" onError={e => (e.currentTarget.style.display = 'none')} />
                          </div>
                        )}
                        <div>
                          <p className="text-[15px] font-medium" style={{ color: THEME.primary }}>{property.developer.name}</p>
                          {property.developer.country && <p className="text-[11px] mt-0.5" style={{ color: THEME.muted }}>{property.developer.country}</p>}
                          {property.developer.website && (
                            <a href={property.developer.website} target="_blank" rel="noreferrer" className="mt-1.5 inline-flex items-center gap-1 text-[10px] underline hover:no-underline" style={{ color: THEME.primary }}>
                  ...
                  {property.agent?.name && (
                    <div className="mt-8 border-t pt-8" style={{ borderColor: THEME.border }}>
                      <p className="text-[9px] uppercase tracking-[0.2em] mb-4" style={{ color: THEME.muted }}>Listing Agent</p>
                      <div className="flex items-center gap-4">
                        {property.agent.photo ? (
                          <img src={fixImageUrl(property.agent.photo)} alt={property.agent.name} className="h-14 w-14 rounded-full object-cover border-2" style={{ borderColor: THEME.border }} />
                        ) : (
                          <div className="flex h-14 w-14 items-center justify-center rounded-full" style={{ backgroundColor: `${THEME.primary}10` }}>
                            <span className="text-[18px] font-medium" style={{ color: THEME.primary }}>{property.agent.name[0]}</span>
                          </div>
                        )}
                        <div>
                          <p className="text-[14px] font-medium" style={{ color: THEME.primary }}>{property.agent.name}</p>
                          {property.agent.phone && (
                            <a href={`tel:${property.agent.phone}`} className="mt-1 flex items-center gap-1.5 text-[12px] transition-colors hover:opacity-70" style={{ color: THEME.muted }}>
                  ...
                  <div className="mt-8 border-t pt-8" style={{ borderColor: THEME.border }}>
                    <BrochureDownload propertyId={property.id} propertyName={displayName} />
                  </div>
                </motion.div>
              )}
                      ...
                      .map((plan, i) => (
                        ...
                        >
                          ...
                          <p className="text-[40px] font-light leading-none" style={{ fontFamily: FONT_DISPLAY, color: THEME.primary }}>
                            {plan.percentage}<span className="text-[24px]">%</span>
                          </p>
                          <p className="mt-3 text-[11px] font-medium uppercase tracking-[0.15em]" style={{ color: THEME.muted }}>{plan.name}</p>
                        </motion.div>
                      ))}
                  </div>
                </motion.div>
      ...
      <div className="mx-auto max-w-[1180px] px-4 pb-12 md:px-6">
        ...
        <div className="overflow-hidden rounded-[3px] border" style={{ borderColor: THEME.border }}>
          ...
          <div className="flex items-center gap-2 px-4 py-3 border-t" style={{ borderColor: THEME.border }}>
            ...
            <p className="text-[12px]" style={{ color: THEME.muted }}>{property.location?.address || displayName}, {location}, Dubai</p>
          </div>
        </div>
      </div>
      ...
      {similarProperties.length > 0 && (
        <div className="border-t mt-16 pt-16" style={{ borderColor: THEME.border }}>
          <div className="mx-auto max-w-[1180px] px-4 pb-16 md:px-6">
            <div className="mb-10 flex items-end justify-between">
              <div>
                <p className="text-[9px] uppercase tracking-[0.25em] mb-2" style={{ color: THEME.accent }}>Explore More</p>
                <h2 className="text-[28px]" style={{ fontFamily: FONT_DISPLAY, color: THEME.primary }}>Similar Apartments</h2>
              </div>
              <Link href="/apartments-for-sale-in-dubai" className="hidden items-center gap-2 text-[10px] uppercase tracking-[0.2em] transition-colors hover:opacity-70 sm:flex" style={{ color: THEME.primary }}>
                View All <ChevronLeft className="h-3.5 w-3.5 rotate-180" />
              </Link>
            </div>
            <div className="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3">
              {similarProperties.map((p, i) => <SimilarCard key={p.id} property={p} index={i} />)}
            </div>
          </div>
        </div>
      )}
