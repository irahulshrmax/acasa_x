...
export async function GET(
  ...
  try {
    ...
    const responseData: PropertyResponse = {
      ...
      is_off_plan: property.listing_type === 'Off plan',
      off_plan_details: {
    ...
    if (includeSimilar) {
      try {
        ...
        if (!similarResult.data.length && property.listing_type) {
          const result = await getProperties({
...
export async function PUT(
  ...
  try {
    ...
    const existingProperty = await getPropertyById(idNum);
    if (!existingProperty) {
      return NextResponse.json(
        { success: false, message: 'Property not found' },
        { status: 404 }
    ...
    if (body.property_slug && body.property_slug !== existingProperty.property_slug) {
      const slugExists = await getPropertyBySlug(body.property_slug);
      if (slugExists && slugExists.id !== idNum) {
        return NextResponse.json(
          { success: false, message: 'Property with this slug already exists' },
          { status: 409 }
        );
      }
    }

    const property = await updateProperty(idNum, body);
    
    clearCache(`property:${id}`);
    clearCache(`property:${existingProperty.property_slug}`);
...
export async function DELETE(
  ...
  try {
    ...
    const result = await deleteProperty(idNum);
    ...
    return NextResponse.json({
      success: true,
      data: result,
      message: permanent ? 'Property permanently deleted' : 'Property moved to archive',
      meta: { 
        deleted: true, 
        permanent,
        timestamp: new Date().toISOString() 
...
export async function PATCH(
  ...
  try {
    ...
    if (action === 'restore') {
      const property = await restoreArchiveProperty(idNum);
      ...
      return NextResponse.json({
        ...
        meta: { action: 'restore', timestamp: new Date().toISOString() },
      });
    }
    ...
    const property = await updateProperty(idNum, updateData);
