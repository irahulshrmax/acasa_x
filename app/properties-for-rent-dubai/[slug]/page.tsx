// app/rent-properties/[slug]/page.tsx
...
const API_URL = "/api/v1/properties/rent";
          ...
          {done ? (
            ...
            >
              ...
              <div>
                ...
                <textarea
                  rows={3}
                  placeholder="I'm interested in this rental property..."
                  value={form.message}
...
function SimilarCard({ property, index }: { property: any; index: number }) {
  ...
  return (
    <motion.div
      ...
      onClick={() => router.push(`/rent-properties/${property.slug}`)}
...
export default function RentPropertyDetailPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const data = await res.json();
        if (!res.ok || !data.success) throw new Error(data.message || "Rental property not found");
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-white">
        <div className="text-center px-4">
          ...
          <p className="text-sm text-red-500 mb-6">{error || "Rental property not found"}</p>
          <Link
            href="/rent-properties"
  ...
  const isRent = property.listing_type === "Rent" || property.is_rent === true;
        ...
        <div className="mx-auto max-w-[1180px] px-4 py-3 md:px-6">
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-1.5 text-[10px] uppercase tracking-[0.1em]" style={{ color: THEME.muted }}>
              ...
              <Link href="/rent-properties" className="hover:text-gray-600 transition-colors">
                Rent Properties
              </Link>
              <ChevronLeft className="h-3 w-3 rotate-180" />
        ...
        <div className="flex flex-wrap items-start justify-between gap-6">
          <div className="flex-1 min-w-0">
            <div className="flex flex-wrap gap-2 mb-3">
              ...
              {isRent && (
                <span
                  className="flex items-center gap-1 px-2.5 py-1 text-[8px] font-medium uppercase tracking-[0.15em] bg-emerald-50 text-emerald-700 border border-emerald-200"
                >
            ...
            {property.developer?.name && (
              <div className="mt-2 flex items-center gap-2 text-[11px]" style={{ color: THEME.muted }}>
                ...
                {property.developer.is_international && (
                  <span className="text-[9px] bg-gray-100 px-2 py-0.5 rounded">International</span>
                )}
              </div>
            )}
          </div>
          ...
          <div className="shrink-0 text-right">
            ...
            {!property.price?.is_price_on_request && property.price?.currency && (
              <p className="mt-1 text-[10px] uppercase tracking-widest" style={{ color: THEME.muted }}>
                {property.price.currency} / year
              </p>
            )}
            {property.price?.sale_price && (
              <p className="mt-1 text-[11px]" style={{ color: THEME.muted }}>
                Sale: {property.price.currency} {property.price.sale_price.toLocaleString()}
              </p>
            )}
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          ...
          {property.dld_permit && (
            <div
              ...
              style={{ borderColor: THEME.border }}
            >
              <Shield className="h-3.5 w-3.5" style={{ color: THEME.accent }} />
              <span className="text-[11px] font-medium" style={{ color: THEME.primary }}>
                ...
                >
                  ...
                  {property.description ? (
                    ...
                    <p className="text-[13px] leading-relaxed" style={{ color: "#4A5462" }}>
                      Welcome to {property.property_name}, a premium rental property
                  ...
                  >
                    {[
                      { label: "Property Type", value: property.listing_type || "Rent" },
