...
function SimilarProperties({ currentId, similarList }: { currentId: number; similarList?: any[] }) {
  ...
  useEffect(() => {
    // If similarList is provided from API, use it
    ...
    (async () => {
      try {
        const res = await fetch(
          `${API_URL}?page=1&limit=6&sort_by=newest&listing_type=Off plan&status=5`
        );
        ...
        <div className="mx-auto max-w-[1180px] px-4 py-3 md:px-6">
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-1.5 text-[10px] uppercase tracking-[0.1em]" style={{ color: THEME.muted }}>
              ...
              <Link href="/properties" className="hover:text-gray-600 transition-colors">
                Properties
              </Link>
              <ChevronLeft className="h-3 w-3 rotate-180" />
        ...
        <div className="flex flex-wrap items-start justify-between gap-6">
          <div className="flex-1 min-w-0">
            <div className="flex flex-wrap gap-2 mb-3">
              ...
              {property.listing_type && (
                <span
                  className={`px-2.5 py-1 text-[8px] font-medium uppercase tracking-[0.15em] border ${
                    isOffPlan ? "bg-emerald-50 text-emerald-700 border-emerald-200" : ""
                  }`}
              ...
              {isOffPlan && property.completion_date && (
          ...
          <div className="shrink-0 text-right">
            ...
            {property.price?.rental_price && !isOffPlan && (
